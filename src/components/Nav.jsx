...
export default function Nav() {
  ...
  const navLinks = [
    ...
    { label: 'XLVI', path: '/xlvi', isNew: true },
    ...
    <>
      ...
      >
          ...
          >
            <motion.img
              ...
              style={{ height: isScrolled ? '56px' : '76px',
                ...
                border: `2px solid ${COLORS.gold}30`, flexShrink: 0 }}
            />
            <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
              <div style={{ fontFamily: SERIF,
                fontSize: isScrolled ? '18px' : '24px',
                fontWeight: 400, fontStyle: 'italic', color: COLORS.gold,
                letterSpacing: '2px', lineHeight: 1,
                transition: 'font-size 0.4s ease', whiteSpace: 'nowrap' }}>
                Bottega Italiana
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div style={{ display: 'flex', gap: '2px' }}>
                  {[COLORS.green, '#999', COLORS.inferno].map((c, i) => (
                    <div key={i} style={{ width: '14px', height: '2px', background: c,
                      opacity: isScrolled ? 0.7 : 1, transition: 'opacity 0.4s ease' }} />
                  ))}
                </div>
                <div style={{ fontFamily: SANS,
                  fontSize: isScrolled ? '8px' : '9px',
                  fontWeight: 500, color: `${COLORS.black}80`,
          ...
          >
            {navLinks.map((link) => (
              <motion.button
                ...
                style={{ background: 'none', border: 'none',
                  ...
                  letterSpacing: '2px', fontWeight: isActive(link.path) ? 600 : 400,
                  color: isActive(link.path) ? COLORS.gold : `${COLORS.black}BB`,
                  cursor: 'pointer', position: 'relative', padding: '10px 0',
                  textTransform: 'uppercase', transition: 'color 0.3s ease, font-size 0.4s ease' }}
                whileHover={{ color: COLORS.gold }}
              >
                {link.label}
                {link.isNew && (
                  <span style={{ position: 'absolute', top: '2px', right: '-16px',
                    background: COLORS.gold, color: COLORS.black,
                    fontSize: '7px', fontWeight: 800, letterSpacing: '0',
                ...
                {isActive(link.path) && (
                  <motion.div
                    ...
                    style={{ position: 'absolute', bottom: 0, left: 0, right: 0,
                      height: '2px', background: COLORS.gold, borderRadius: '1px' }}
            ...
            <motion.a
              ...
              style={{ background: COLORS.gold, color: COLORS.black,
                ...
                display: 'inline-block', transition: 'all 0.4s ease', whiteSpace: 'nowrap' }}
              className="nav-cta"
            >
              Contactar
            </motion.a>
        ...
        <motion.div
          ...
          style={{ height: '2px', background: COLORS.gold, width: '100%', transformOrigin: 'left' }}
        />
      </motion.nav>
      ...
      <AnimatePresence>
        {isMobileOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            transition={{ duration: 0.3 }}
            style={{ position: 'fixed', inset: 0, background: COLORS.snow,
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '50px' }}>
              ...
              <div>
                <div style={{ fontFamily: SERIF, fontSize: '20px', fontStyle: 'italic',
                  color: COLORS.gold, letterSpacing: '2px' }}>Bottega Italiana</div>
              ...
              {navLinks.map((link, i) => (
                <motion.button
                  ...
                  style={{ background: 'none', border: 'none', textAlign: 'left',
                    padding: '16px 0', borderBottom: `1px solid ${COLORS.gold}20`,
                    cursor: 'pointer', display: 'flex',
                    alignItems: 'center', justifyContent: 'space-between' }}
                >
                  <span style={{ fontFamily: SERIF, fontSize: '2rem',
                    fontStyle: 'italic', fontWeight: 300, letterSpacing: '1px',
                    color: isActive(link.path) ? COLORS.gold : COLORS.black }}>
                    {link.label}
                    {link.isNew && (
                      <span style={{ fontSize: '0.7rem', fontStyle: 'normal',
                        background: COLORS.gold, color: COLORS.black,
                        padding: '2px 8px', marginLeft: '12px',
                        fontFamily: SANS, fontWeight: 800, letterSpacing: '0' }}>
                        NEW
                      </span>
                    )}
                  </span>
            ...
            <motion.a
              ...
              href="https://wa.me/59178594506" target="_blank" rel="noopener noreferrer"
              style={{ display: 'block', background: '#0C7A2A', color: COLORS.snow,
                textAlign: 'center', padding: '18px', fontFamily: SANS,
            ...
            <motion.button
              ...
              style={{ position: 'absolute', top: '24px', right: '28px',
                ...
                color: COLORS.black, fontSize: '24px', lineHeight: 1, padding: '8px' }}
              whileTap={{ scale: 0.9 }}
            >
              ✕
            </motion.button>
      ...
      <style>{`
        @media (max-width: 1100px) { .nav-cta { display: none !important; } }
